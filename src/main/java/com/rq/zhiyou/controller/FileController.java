...
import com.rq.zhiyou.constant.FileConstant;
...
public class FileController {
     ...
     */
    @PostMapping("/upload")
    public ResultData<String> uploadFile(@RequestPart("file") MultipartFile multipartFile,
        ...
        try {
            ...
            // 返回可访问地址
            return ResultData.success(FileConstant.COS_HOST + filepath);
        } catch (Exception e) {
    ...
    private void validFile(MultipartFile multipartFile, FileUploadBizEnum fileUploadBizEnum) {
        ...
        if (FileUploadBizEnum.USER_AVATAR.equals(fileUploadBizEnum)) {
            if (fileSize > ONE_M) {
                throw new BusinessException(StatusCode.PARAMS_ERROR, "文件大小不能超过 5M");
            }
            if (!Arrays.asList("jpeg", "jpg", "svg", "png", "webp","gif").contains(fileSuffix)) {
                throw new BusinessException(StatusCode.PARAMS_ERROR, "文件类型错误");
            }
        }
    }
}
